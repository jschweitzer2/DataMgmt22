...
local dir ~/Desktop/DataMgmtPS5  // Local marco with directory path
...
log using Schweitzer_PS5.txt, replace   // Create log of session
...
local HSAcademics HSGPA SAT_M SAT_V
...
regress FY_GPA `HSAcademics'
estimates store Model_2

regress FY_GPA `HSAcademics' `Demographics'
estimates store Model_3

regress FY_GPA `HSAcademics' `Demographics' `Residency'
estimates store Model_4

regress FY_GPA `HSAcademics' `Demographics' `Residency' LNHOUSEHOLDAGI
estimates store Model_5

regress FY_GPA `HSAcademics' `Demographics' `Residency' `FinAid'
...
// Nested loop to run a seperate regression for each academic college by year to review year-to-year differences in estimates per school

levelsof nCollege, local(college)

foreach level in `college' {
	forvalues YR = 2018/2020 {
	display `YR'
	display `level'
	regress FY_GPA `HSAcademics' `Demographics' if YR == `YR' & nCollege == `level'
}
}
